...
class PolicyAgent:
    def __init__(self, n, obs_shape, n_actions, lr_value=0.001, lr_policy=0.001, gamma=0.99, beta=0.01,
                 device="cpu", preprocessor=None, load_path=None):
        self.n = n
        ...
        self.optimizer_policy = optim.Adam(self.policy_net.parameters(), lr=lr_policy, eps=1e-3)
        self.gamma = gamma
        self.beta = beta
        self.device = device
        ...
        if load_path is not None:
            ...
            self.value_net.load_state_dict(torch.load(load_path+"value.pt", map_location=torch.device(device)))
            self.policy_net.load_state_dict(torch.load(load_path+"policy.pt", map_location=torch.device(device)))
    ...
    @torch.no_grad()
    def act(self, states, temperature=1):
        states = self.list_2_tensor(states)
        logits = temperature*self.policy_net(states)
    ...
    def save_model(self, path):
        torch.save(self.value_net.state_dict(), path + "value.pt")
        torch.save(self.policy_net.state_dict(), path + "policy.pt")
